...
test("mounts one part while keeping part and section navigation distinct", async () => {
  ...
  assert.doesNotMatch(page, /Temporary animation lab|motion-readout|Reset animation defaults/);
  assert.doesNotMatch(page, /Move settings window|settingsDrag|settingsPosition/);
  assert.match(css, /\.floating-card[^}]*position:\s*fixed/);
  assert.doesNotMatch(css, /settings-drag-handle/);
  assert.match(css, /\.reader-shell[^}]*--part-motion-duration:\s*270ms[^}]*--part-motion-distance:\s*92px[^}]*--part-motion-opacity:\s*\.05[^}]*--part-motion-easing:\s*ease-in-out/);
